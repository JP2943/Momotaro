...
## PC 側シェルからコミットするとき（P5-03b で踏んだ）

`git add` と `git commit` は索引を書くので、上の「ロックを作らない」は当てはまらない。
**ロックが残るのは織り込み済みとし、削除は所有者が手で行う**（2026-09-23 に所有者と合意）。
ロックを消すために Claude が時間を使わない。次の形で実行する。

```
git --no-optional-locks \
  -c user.name="<直前のコミットの著者名>" -c user.email="<同メール>" \
  -c core.hooksPath=/dev/null \
  commit -q --no-verify -F <メッセージファイル>
```

- **著者情報が無い。** PC 側シェルの git には `user.name` / `user.email` が設定されていない
  （Windows 側の設定は見えない）。値は `git log -1 --format='%an <%ae>'` で直前のコミットから採る。
- **LFS の post-commit フックが落ちる。** git-lfs が入っていないのでフックがエラーを返すため、
  `core.hooksPath` を無効にする。
- **コミットに 3 分以上かかり、ツールのタイムアウトで殺されることがある。** 殺されても
  **コミット自体は成立していることが多い**。タイムアウトしたら、**まず `git log --oneline -2` で
  入っているか確かめる**。入っていれば再実行しない（二重コミットになる）。
- **残ったロックは `_to_delete/git_locks/` へ `mv` して退避し、報告して終える。**
  PC 側シェルは削除できないので、所有者が Windows 側で消す。
  `.git/objects/**/tmp_obj_*` も同様に残るが、Windows 側の `git gc` で片付く。
- ロックが残ったままだと**次のコミットが `File exists` で即座に失敗する**。
  退避してから次のコミットへ進むこと。
