...
namespace Momotaro.Editor.Phase5
{
    ...
    public static class Phase5ExplorationValidator
    {
        ...
        public static void Validate(Scene scene, List<string> errors, List<string> warnings)
        {
            ...
            ValidateNavigation(scene, errors);
        ...
        private static void ValidateNavigation(Scene scene, List<string> errors)
        {
            ...
            // 供給元そのものは実行時に注入される（Scene には保存されない）ので、静的に見えるのは
            // <b>注入する役が居て配線されているか</b>まで。未注入のまま走り出したときの停止・診断は
            // §10.1 のとおり実行時の責務で、P5-06 の受入が実 Scene で見ている。
        }
        ...
        private static void ValidateSceneHygiene(Scene scene, List<string> errors)
        {
            foreach (GameObject root in scene.GetRootGameObjects())
            {
                ...
                if (missing > 0)
                {
                    errors.Add("Missing Script が " + missing + " 個あります（" + root.name + " 以下）。");
                }
