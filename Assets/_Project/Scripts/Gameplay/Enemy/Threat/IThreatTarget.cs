...
namespace Momotaro.Gameplay.Enemy.Threat
{
    ...
    public interface IThreatTarget : IPerceptionTarget
    {
        ...
        /// <summary>
        /// 獲得ヘイトの上限（試遊フィードバック 2026-09-21 その 2）。<b>0 以下は無制限</b>。
        ///
        /// 仲間は殴り続けるかぎり獲得ヘイトが伸び続けるため、上限が無いと「先に横取りした仲間がその戦闘を最後まで抱え込み、
        /// 主人公は殴り放題」という難易度低下が起きる。上限を置くと仲間の脅威に天井ができ、主人公が本気で殴れば
        /// 必ず狙いが戻る。主人公は 0（無制限）を基本とし、仲間だけが天井を持つ。
        ///
        /// 既定実装は 0（無制限）。天井を持たせたい対象だけが明示的に上書きする。
        /// </summary>
        float MaxAcquiredThreat => 0f;
    }
}
